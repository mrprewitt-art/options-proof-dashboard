...
CREATE TABLE IF NOT EXISTS closed_trades(
 ...
 trade_lane TEXT,strike TEXT,expiry TEXT,entry_price DOUBLE PRECISION NOT NULL,
 tp1_price DOUBLE PRECISION,tp1_return_pct DOUBLE PRECISION,tp1_size_pct DOUBLE PRECISION,tp1_at TIMESTAMPTZ,
 tp2_price DOUBLE PRECISION,tp2_return_pct DOUBLE PRECISION,tp2_size_pct DOUBLE PRECISION,tp2_at TIMESTAMPTZ,
...
class ClosedTradeIn(BaseModel):
    ...
    tp1_price:Optional[float]=Field(default=None,ge=0); tp1_return_pct:Optional[float]=None; tp1_size_pct:Optional[float]=Field(default=None,ge=0,le=100); tp1_at:Optional[str]=None
    tp2_price:Optional[float]=Field(default=None,ge=0); tp2_return_pct:Optional[float]=None; tp2_size_pct:Optional[float]=Field(default=None,ge=0,le=100); tp2_at:Optional[str]=None
    exit_price:float=Field(ge=0); exit_return_pct:Optional[float]=None; pnl_pct:float
    opened_at:Optional[str]=None; closed_at:str; source_trade_id:str; notes:Optional[str]=None

def _stats(conn):
    row=conn.execute("SELECT COUNT(*),COUNT(*) FILTER(WHERE pnl_pct>0),COUNT(*) FILTER(WHERE pnl_pct<=0),COALESCE(AVG(pnl_pct),0),COALESCE(MAX(pnl_pct),0) FROM closed_trades WHERE source_trade_id LIKE %s",(SOURCE_PREFIX+"%",)).fetchone()
...
def dashboard_data(limit=50):
    with connect() as conn:
        ...
        rows=conn.execute("""SELECT ticker,option_type,trade_lane,strike,expiry,entry_price,
          tp1_price,tp1_return_pct,tp1_size_pct,tp1_at,tp2_price,tp2_return_pct,tp2_size_pct,tp2_at,
          exit_price,COALESCE(exit_return_pct,pnl_pct),pnl_pct,opened_at,closed_at,notes
          FROM closed_trades WHERE source_trade_id LIKE %s ORDER BY closed_at DESC,id DESC LIMIT %s""",(SOURCE_PREFIX+"%",limit)).fetchall()
    cols=["ticker","option_type","trade_lane","strike","expiry","entry_price","tp1_price","tp1_return_pct","tp1_size_pct","tp1_at","tp2_price","tp2_return_pct","tp2_size_pct","tp2_at","exit_price","exit_return_pct","pnl_pct","opened_at","closed_at","notes"]
...
async def closed_trade(trade:ClosedTradeIn,x_trade_secret:Optional[str]=Header(default=None)):
    _auth(x_trade_secret); source_id=trade.source_trade_id.strip(); typ=trade.option_type.strip().upper()
    if not source_id.startswith(SOURCE_PREFIX): raise HTTPException(400,"Only Sniper Production closed trades are accepted")
    if typ not in {"CALL","PUT"}: raise HTTPException(400,"option_type must be CALL or PUT")
    with connect() as conn:
        row=conn.execute("""INSERT INTO closed_trades(source_trade_id,ticker,option_type,trade_lane,strike,expiry,entry_price,
          tp1_price,tp1_return_pct,tp1_size_pct,tp1_at,tp2_price,tp2_return_pct,tp2_size_pct,tp2_at,
          exit_price,exit_return_pct,pnl_pct,opened_at,closed_at,notes)
          VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,NULLIF(%s,'')::timestamptz,%s,%s,%s,NULLIF(%s,'')::timestamptz,%s,%s,%s,NULLIF(%s,'')::timestamptz,%s::timestamptz,%s)
          ON CONFLICT(source_trade_id) DO NOTHING RETURNING id""",
          (source_id,trade.ticker.strip().upper(),typ,trade.trade_lane,trade.strike,trade.expiry,float(trade.entry_price),
           trade.tp1_price,trade.tp1_return_pct,trade.tp1_size_pct,trade.tp1_at or "",trade.tp2_price,trade.tp2_return_pct,trade.tp2_size_pct,trade.tp2_at or "",
